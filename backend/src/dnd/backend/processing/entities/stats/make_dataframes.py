...
def read_char_inputs() -> pd.DataFrame:
    character_df: pd.DataFrame = pd.read_csv(char_in_dir, header=None) # type: ignore
    character_df = character_df.rename(columns=character_df.iloc[0]).drop(character_df.index[0]) # type: ignore
    character_df.set_index('Name', inplace=True) # type: ignore
    return character_df


def read_armour_inputs() -> pd.DataFrame:
    armour_df: pd.DataFrame = pd.read_csv(armour_in_dir, header=None) # type: ignore
    armour_df = armour_df.rename(columns=armour_df.iloc[0]).drop(armour_df.index[0]) # type: ignore
    armour_df.set_index('Name', inplace=True) # type: ignore
    return armour_df


def read_weapon_inputs() -> pd.DataFrame:
    weapon_df: pd.DataFrame = pd.read_csv(weapon_in_dir, header=None) # type: ignore
    weapon_df = weapon_df.rename(columns=weapon_df.iloc[0]).drop(weapon_df.index[0]) # type: ignore
    weapon_df.set_index('Name', inplace=True) # type: ignore
    return weapon_df


def make_character_stat_table(map_number_str: str):
...
class EntityTables:
    ...
    def get_character_stats_dict(self, character_name: str) -> Dict[str, object]:
        # Make a dictionary of the stats for the entity
        gotten_stats: Dict[str, object] = self.characters.loc[character_name].to_dict() # type: ignore
        return gotten_stats

    def get_armour_stats_dict(self, armour_name: str) -> Dict[str, str]:
        # Make a dictionary of the stats for the entity
        gotten_stats: Dict[str, str] = self.armour.loc[armour_name].to_dict() # type: ignore
        return gotten_stats


def make_raw_attack_stat_table() -> pd.DataFrame:
...
def make_weapon_attack_stat_table() -> pd.DataFrame:
